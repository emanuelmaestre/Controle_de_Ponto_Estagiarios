...
export async function GET(request: Request) {
  ...
  if (report === 'attendance') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Data',           dataKey: 'data',     width: 16 },
        { header: 'Dia',            dataKey: 'dia',      width: 10 },
        { header: 'Entrada / Saída', dataKey: 'horario', width: 34 },
        { header: 'Total',          dataKey: 'total',    width: 14 },
  ...
  if (report === 'hours') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Data',     dataKey: 'data',   width: 24 },
        { header: 'Entrada',  dataKey: 'entrada', width: 22 },
        { header: 'Saída',    dataKey: 'saida',   width: 22 },
        { header: 'Duração',  dataKey: 'duracao', width: 20 },
        { header: 'Status',   dataKey: 'status',  width: 24 },
      ],
      totals: [
  ...
  if (report === 'punctuality') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Data',    dataKey: 'data',    width: 30 },
        { header: 'Entrada', dataKey: 'entrada', width: 28 },
        { header: 'Pontual', dataKey: 'pontual', width: 22 },
  ...
  if (report === 'activities') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Data',      dataKey: 'data',      width: 26 },
        { header: 'Atividade', dataKey: 'atividade' },
      ],
      rows,
    })
  ...
  if (report === 'summary') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Estagiário', dataKey: 'nome',      width: 52 },
        { header: 'Curso',      dataKey: 'curso',     width: 42 },
        { header: 'Horas',      dataKey: 'horas',     width: 20 },
        { header: 'Sessões',    dataKey: 'sessoes',   width: 18 },
        { header: 'Aprovadas',  dataKey: 'aprovadas', width: 20 },
        { header: 'Atrasos',    dataKey: 'atrasos',   width: 18 },
      ],
      totals: [
  ...
  if (report === 'daily_log') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Estagiário', dataKey: 'nome',    width: 44 },
        { header: 'Data',       dataKey: 'data',    width: 20 },
        { header: 'Dia',        dataKey: 'dia',     width: 12 },
        { header: 'Entrada',    dataKey: 'entrada', width: 20 },
        { header: 'Saída',      dataKey: 'saida',   width: 20 },
        { header: 'Duração',    dataKey: 'duracao', width: 18 },
  ...
  if (report === 'by_course') {
    ...
    return NextResponse.json({
      ...
      columns: [
        ...
        { header: 'Horas',        dataKey: 'horas',        width: 24 },
        { header: 'Sessões',      dataKey: 'sessoes',      width: 22 },
        { header: 'Estagiários',  dataKey: 'estagiarios',  width: 28 },
      ],
      rows,
    })
  }
  ...
  if (report === 'ending_soon') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: 'Estagiário',  dataKey: 'nome',     width: 50 },
        { header: 'Curso',       dataKey: 'curso',    width: 40 },
        { header: 'Cumprido',    dataKey: 'cumprido', width: 24 },
        { header: 'Restante',    dataKey: 'restante', width: 24 },
        { header: 'Carga Total', dataKey: 'total',    width: 22 },
        { header: 'Conclusão',   dataKey: 'pct',      width: 18 },
      ],
      rows,
    })
  }
  ...
  if (report === 'ranking_full') {
    ...
    return NextResponse.json({
      ...
      columns: [
        ...
        { header: 'Curso',  dataKey: 'curso',  width: 48 },
        { header: 'Nível',  dataKey: 'nivel',  width: 18 },
        { header: 'Pontos', dataKey: 'pontos', width: 22 },
      ],
      rows,
    })
  }
  ...
  if (report === 'ranking_punctuality') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: '#',        dataKey: 'pos',    width: 12 },
        { header: 'Nome',     dataKey: 'nome',   width: 68 },
        { header: 'Pontuais', dataKey: 'ontime', width: 24 },
        { header: 'Atrasos',  dataKey: 'late',   width: 22 },
        { header: 'Taxa',     dataKey: 'taxa',   width: 20 },
      ],
      rows,
    })
  }
  ...
  if (report === 'hall_of_fame') {
    ...
    return NextResponse.json({
      ...
      columns: [
        { header: '#',        dataKey: 'pos',    width: 12 },
        { header: 'Nome',     dataKey: 'nome',   width: 65 },
        { header: 'Pontos',   dataKey: 'pontos', width: 24 },
        { header: 'Nível',    dataKey: 'nivel',  width: 18 },
        { header: 'Sequência', dataKey: 'streak', width: 25 },
      ],
      rows,
    })
  }
