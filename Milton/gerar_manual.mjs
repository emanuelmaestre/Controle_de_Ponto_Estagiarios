...
const sec3 = [
  ...
  tbl2([
    ...
    ['src/components/', 'Componentes reutilizáveis (OnboardingTour, ClockButton, GeofenceWatcher, InternNotificationBell, etc.)'],
...
const sec4 = [
  ...
  tbl2([
    ['Tabela', 'Descrição'],
    ['profiles', 'Perfil do usuário: full_name, email, role, photo_url, points, level, streak_days, geo_exempt, pin (bcrypt hash), is_active, total_hours_required, course_duration_years'],
    ['time_records', 'Registros de ponto: clock_in, clock_out, duration_minutes, status (pending/approved/rejected), rejection_reason, notes (ex: "auto_close_geo"), geo_lat, geo_lng'],
...
const sec5 = [
  ...
  tbl2([
    ...
    ['/api/clock/auto-out', 'POST — fecha registro em aberto sem exigir atividades (geofence auto-checkout). Marca notes="auto_close_geo". Trigger: componente GeofenceWatcher no dashboard.'],
  ]),
  ...
  tbl2([
    ...
    ['/api/intern/notifications', 'GET — retorna lista de notificações persistentes para o sino do estagiário (polling a cada 60 s). Inclui: cadastro incompleto, ponto em aberto, feedback respondido, conquistas, metas de horas e atividades pendentes de registros auto-fechados por geofence.'],
  ]),
...
const sec7 = [
  ...
  p(t('Os limiares de pontos dos níveis são proporcionais à duração do curso (course_duration_years). Os valores abaixo correspondem ao padrão de 5 anos:')),
  tbl2([
    ['Nível', 'Título', 'Pontos necessários (5 anos)'],
  ...
  p(t('Para cursos de duração diferente, o fator de escala é aplicado automaticamente: ex. curso de 2 anos usa ×0,4 nos limiares. O campo course_duration_years é preenchido automaticamente ao selecionar o nome do curso no cadastro.')),
...
const sec10 = [
  ...
  tbl2([
    ...
    ['/dashboard', 'Painel principal: registro de ponto, progresso, gamificação, onboarding tour e monitoramento de geofence (GeofenceWatcher)'],
    ['/history', 'Histórico dos últimos 60 dias + arquivo por mês. Sino de notificações no header.'],
    ...
    ['/profile', 'Edição de dados, conquistas, relatórios, novidades e reativação do tour. Sino de notificações no header.'],
...
const sec14 = [
  ...
  h2('14.6 Geofence Auto-Checkout'),
  bullet('O componente GeofenceWatcher (src/components/GeofenceWatcher.tsx) monitora a posição do estagiário durante a sessão no dashboard'),
  bullet('Se o estagiário sair a mais de 200 m do laboratório com registro aberto, o sistema chama /api/clock/auto-out automaticamente'),
  bullet('O registro é fechado com notes="auto_close_geo" e sem atividades registradas'),
  bullet('O sino de notificações exibe alerta persistente pedindo que o estagiário registre as atividades via Histórico'),
  bullet('Estagiários com geo_exempt=true ou sem registro aberto não são monitorados'),
  bullet('Para ajustar o raio: altere geo_radius_meters em /admin/settings (padrão operacional do clock-in) ou a prop radiusMeters do GeofenceWatcher (padrão 200 m)'),
  h2('14.7 Monitorar Erros em Produção'),
  ...
  h2('14.8 Atualizar Tipos do Banco'),
